...
public class AdminStoryHistoryController : MonoBehaviour
{
    ...
    [Header("HISTORY")]
    [SerializeField] private List<AdminStoryHIstoryItem> historyItems;
    ...
    private void CallHistoryAPI()
    {
        ...
        HistoryAPI = StartCoroutine(apiController.GetRequest("/story/viewassessmenthistory", $"?page={paginationIndex}&limit=10", false, (response) =>
        {
            if (response.ToString() != "")
            {
                StoryAdminHistory tempdata = JsonConvert.DeserializeObject<StoryAdminHistory>(response.ToString());
                ...
                else
                {
                    for (int a = 0; a < historyItems.Count; a++)
                    {
                        if (a < tempdata.history.Length)
                            historyItems[a].HistoryData($"{tempdata.history[a].storyid}", $"{tempdata.history[a].createdAt}", $"{tempdata.history[a].title}", $"{tempdata.history[a].fullname}", tempdata.history[a].score);
                        else
                            historyItems[a].HistoryData("", "", "NO DATA YET!", "", 0);
                    }
                }
...
[System.Serializable]
public class StoryAdminHistory
{
    public StoryAdminHistoryData[] history;
    public int totalpages;
}

[System.Serializable]
public class StoryAdminHistoryData
{
